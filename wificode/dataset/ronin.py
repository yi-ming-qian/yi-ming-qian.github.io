...
class Ronin(object):
	...
	def readFromText(self, read_gyro=True, bssid_database=None, read_wifi=True, read_tango=True, read_flp=True):
		# ronin
	...
	def read_tango_txt(self):
		self.tango_times, self.tango_XYs = [], []
		with open(self.data_path+'tangoXY.txt', 'r') as f:
			for line in f:
				line = line.strip()
				tokens = [float(token) for token in line.split(' ') if token.strip() != '']
				self.tango_times.append(tokens[0])
				self.tango_XYs.append(tokens[1:3])
		self.tango_times = np.asarray(self.tango_times)
		self.tango_XYs = np.asarray(self.tango_XYs)
		self.sample_tango_wifi()
		
		#print(self.tango_times.shape, self.tango_XYs.shape, self.tango_has_wifi.shape)
		# xflag = np.sum(np.absolute(locations[:,0]-58.3775)<1e-3)>0
		# yflag = np.sum(np.absolute(locations[:,1]-16.2309)<1e-3)>0
		# if xflag and yflag:
		# 	print(folder_name)
			...
			for i, line in enumerate(f):
				...
				if len(tokens)==1:
					if len(stamp_segments)!=0:
						if np.sum(np.isnan(rssi_segments)==False)>1: # if all nan, don't store it
	...
	def read_flp_txt(self):
		...
		with open(self.data_path+"FLP.txt") as f:
			for i, line in enumerate(f):
				...
				line = line.strip()
				tokens = [float(token) for token in line.split(' ') if token.strip() != '']
				self.flp_times_full.append(tokens[0]*1e-9)
				latlon.append(tokens[1:3])
		self.flp_times_full = np.asarray(self.flp_times_full)
		latlon = np.asarray(latlon)
		tmp = utm.from_latlon(latlon[:,0], latlon[:,1])
		self.flp_locs_full = np.zeros_like(latlon)
		self.flp_locs_full[:,0] = tmp[0]
		self.flp_locs_full[:,1] = tmp[1]
		# scale = np.cos(latlon[0,0] * np.pi / 180.0)
		# er = 6378137
		# self.flp_locs_full[:,0] = scale * latlon[:,1] * np.pi * er / 180
		# self.flp_locs_full[:,1] = scale * er * np.log( np.tan((90+latlon[:,0]) * np.pi / 360) )
		if self.read_tango:
			self.sample_tango_flp()

	def visualize_flp(self):
		fig = plt.figure()
		ax = fig.add_subplot(111)
		ax.scatter(self.flp_locs_full[:,0], self.flp_locs_full[:,1], color=(1,0,0), s=2)
		ax.axis('equal')
		plt.tight_layout()
	...
	def sample_tango_wifi(self):
		offset = 0
		wifi_corres_id = np.full(self.tango_times.shape, -1)
		for w, wt in enumerate(self.wifi_times_full):
			offset = np.argmin(np.absolute(self.tango_times[offset:]-wt))+offset
			wifi_corres_id[offset] = w
		self.tango_has_wifi = wifi_corres_id>=0
		self.tango_rssi = self.wifi_rssis_full[wifi_corres_id[self.tango_has_wifi],:]
		self.tango_rssi[np.isnan(self.tango_rssi)] = -102

		self.tango_rssi = self.tango_rssi+102
		self.tango_rssi = np.maximum(0, self.tango_rssi)
		self.ntango_rssi = normalize(self.tango_rssi)
	def sample_tango_flp(self):
		offset = 0
		tango_times = self.tango_times[self.tango_has_wifi]
		self.tango_flp = np.zeros((tango_times.shape[0],2))
		for w, wt in enumerate(tango_times):
			offset = np.argmin(np.absolute(self.flp_times_full[offset:]-wt))+offset
			self.tango_flp[w,:] = self.flp_locs_full[offset]
